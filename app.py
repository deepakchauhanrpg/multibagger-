...
if scan_clicked:
    if not auto_universe:
        ...
        if results:
            ...
            st.error("No data could be fetched for any ticker. Check your network/ticker list and try again.")    current_volume = info.get('volume', 1) or 1
    volume_shock = current_volume / avg_volume if avg_volume > 0 else 1
...
if scan_clicked:
    if not auto_universe:
        ...
        if results:
            ...
            styled_df = df.style.applymap(style_verdict, subset=['AI Action Verdict'])
            st.dataframe(styled_df, use_container_width=True)
